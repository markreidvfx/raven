...
// data. For a Knob widget, p_min and p_max are optional. Read code of e.g.
// KnobFloat(), KnobInt() etc. or examples in 'Demo->Widgets->Data Types' to
...
bool KnobScalar(
    ...
    if (!temp_input_is_active) {
        ...
        if (focus_requested || clicked || double_clicked
            ...
            g.ActiveIdUsingNavDirMask = (1 << ImGuiDir_Left)
                | (1 << ImGuiDir_Right);
    ...
    case ImGuiDataType_S8: {
        t = ((ImS32) * (ImS8*)p_data - *(const ImS8*)p_min)
            / (float)(*(const ImS8*)p_max - *(const ImS8*)p_min);
    };
        break;
    case ImGuiDataType_U8: {
        t = ((ImU32) * (ImU8*)p_data - *(const ImU8*)p_min)
            / (float)(*(const ImU8*)p_max - *(const ImU8*)p_min);
    };
        break;
    case ImGuiDataType_S16: {
        t = ((ImS32) * (ImS16*)p_data - *(const ImS16*)p_min)
            / (float)(*(const ImS16*)p_max - *(const ImS16*)p_min);
    };
        break;
    case ImGuiDataType_U16: {
        t = ((ImU32) * (ImU16*)p_data - *(const ImU16*)p_min)
            / (float)(*(const ImU16*)p_max - *(const ImU16*)p_min);
    };
        break;
    case ImGuiDataType_S32: {
        t = (*(ImS32*)p_data - *(const ImS32*)p_min)
            / (float)(*(const ImS32*)p_max - *(const ImS32*)p_min);
    };
        break;
    case ImGuiDataType_U32: {
        t = (*(ImU32*)p_data - *(const ImU32*)p_min)
            / (float)(*(const ImU32*)p_max - *(const ImU32*)p_min);
    };
        break;
    case ImGuiDataType_S64: {
        t = (*(ImS64*)p_data - *(const ImS64*)p_min)
            / (float)(*(const ImS64*)p_max - *(const ImS64*)p_min);
    };
        break;
    case ImGuiDataType_U64: {
        t = (*(ImU64*)p_data - *(const ImU64*)p_min)
            / (float)(*(const ImU64*)p_max - *(const ImU64*)p_min);
    };
        break;
    case ImGuiDataType_Float: {
        t = (*(float*)p_data - *(const float*)p_min)
            / (float)(*(const float*)p_max - *(const float*)p_min);
    };
        break;
    case ImGuiDataType_Double: {
        t = (*(double*)p_data - *(const double*)p_min)
            / (float)(*(const double*)p_max - *(const double*)p_min);
    };
        break;
    ...
    dl->AddCircleFilled(
        center,
        w / 4,
        IM_COL32_BLACK); // ImGui::GetColorU32(ImGuiCol_FrameBg));
